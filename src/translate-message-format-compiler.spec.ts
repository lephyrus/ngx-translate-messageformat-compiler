...
describe('TranslateMessageFormatCompiler', () => {
  const toCharCodes = (value: String) => Array.from(value).map(char => char.charCodeAt(0));
  ...
  describe('constructor', () => {
    it('should configure the messageformat instance with default values', () => {
      ...
      // BiDiSupport: false
      const result = compiler.compile('{0} >> {1} >> {2}', 'en')(['a', '\u05d1', '\u05d2']);
      expect(toCharCodes(result))
        .toEqual([97, 32, 62, 62, 32, 1489, 32, 62, 62, 32, 1490]);

      // IntlSupport: false
      expect(() => compiler.compile('Unix time started on {T, date, full}', 'en'))
      ...
      expect(compiler.compile(pastryMsg, 'en')({ X: 3, P: 'pie' }))
        .toBe('3 pies');
    });
    ...
    it('should respect passed-in biDiSupport value', () => {
      ...
      result = compiler.compile('{0} >> {1} >> {2}', 'en')(['a', '\u05d1', '\u05d2']);
      expect(toCharCodes(result))
        .toEqual([8206, 97, 8206, 32, 62, 62, 32, 8206, 1489, 8206, 32, 62, 62, 32, 8206, 1490, 8206]);
    ...
    it('should respect passed-in intlSupport value', () => {
      ...
      expect(compiler.compile('Unix time started on {T, date, full}', 'en')({ T: 0 }))
    ...
    it('should respect passed-in strictNumberSign value', () => {
      ...
      compiler = new TranslateMessageFormatCompiler({ strictNumberSign: false });
      expect(compiler.compile(pastryMsg, 'en')({ X: 3, P: 'pie' }))
        .toBe('3 pies');
    });
  });
